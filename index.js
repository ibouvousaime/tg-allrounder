...
`;
      ...
      case "/8ball":
        ...
        handleResponse(response, msg, chatId, myCache, bot, null).catch(
          (err) => {
            console.error(err);
          }
        );
      ...
      case "/currencyConvert":
        ...
        if (!currencyFrom) {
          handleResponse(
            "Missing input currency. Example command : /cc 25000 AED to USD.",
            msg,
            chatId,
            myCache,
            bot,
      ...
      case "/weatherf":
        ...
        getWeather(locationF, "F")
          .then(async (weatherData) => {
            handleResponse(weatherData, msg, chatId, myCache, bot, "pre").catch(
              (err) => {
                console.error(err);
              }
            );
      ...
      case "/etymology":
        ...
        getWordEtymology(etymologyQuery, lang)
          .then((response) => {
            if (response.length)
              handleResponse(
                response.replace(ansiEscapeRegex, ""),
                msg,
                chatId,
                myCache,
                bot,
      ...
      case "/addsticker":
        if (msg.reply_to_message && msg.reply_to_message.photo) {
          ...
          if (!isEmojiString(emojis) && emojis.trim().length == 0) {
            handleResponse(
              "Correct usage: /addsticker <emojis>. Example : /addsticker 💧🍉.",
              msg,
              chatId,
              myCache,
              bot,
      ...
      case "/dream":
        ...
        sendSimpleRequestToClaude(
          `Pretend you're a dream interpreter and interpret this dream: ${dreamData}`
        ).then((response) => {
          handleResponse(
      ...
      case "/oracle":
        ...
        explainContextClaude(
          db.collection("books"),
          `${target?.length > 0 ? target : "@" + msg.from.username}`
        )
          .then((context) => {
            handleResponse(
              `<blockquote expandable>${context}</blockquote>`,
              msg,
              chatId,
              myCache,
              bot,
          ...
          .then(async (results) => {
            ...
            if (results.length) {
              ...
              for (let element of results) {
                const author = (await bot.getChatMember(chatId, element.author))
                  .user;
      ...
      case "/tldr":
        ...
        const repliedToMessageURL = extractUrl(
        ...
        import("@extractus/article-extractor").then(async ({ extract }) => {
          try {
            ...
            handleResponse(
              `<blockquote expandable>${summary.content[0].text}</blockquote>`,
              msg,
              chatId,
              myCache,
              bot,
...
function getFormattedWeatherData(item) {
  ...
  resultLines.push(
    `Current Temperature: ${item.current.temperature}°${item.location.degreetype}`
  );
  resultLines.push(`Humidity: ${item.current.humidity}%`);
  if (item.current.feelslike)
    resultLines.push(
      `Feels Like: ${item.current.feelslike}°${item.location.degreetype}`
    );
  resultLines.push(`Conditions: ${item.current.skytext}`);
  resultLines.push(
    `Observation Time: ${item.current.observationtime} on ${item.current.date}`
  );
  resultLines.push(`Wind: ${item.current.winddisplay}`);
  resultLines.push(
    `High: ${item.forecast[0].high}°${item.location.degreetype}`
  );
  resultLines.push(`Low: ${item.forecast[0].low}°${item.location.degreetype}`);
  if (item.current.dewPt)
    resultLines.push(
      `Dew point: ${item.current.dewPt}°${item.location.degreetype}`
