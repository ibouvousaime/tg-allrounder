...
function findSpotifyTrackLink(text) {
  const spotifyRegex =
    /(?:https?:\/\/open\.spotify\.com\/|spotify:)(track|album)[:\/]([a-zA-Z0-9]+)/;
...
async function getSpotifyMusicLink(link) {
  ...
  if (fullLink) {
    ...
    await ytmusic.initialize({
      cookies: fs
        .readFileSync(path.join(os.homedir(), "cookies.txt"))
        .toString(),
    });
...
async function extractAudioFromVideo(inputVideoPath, callback) {
  if (!fs.existsSync(inputVideoPath)) {
    throw new Error("Input video does not exist");
  }

  const tmpId = crypto.randomUUID();
  const tempDir = path.join(os.tmpdir(), `audioextract-${tmpId}`);
  const outputAudioPath = path.join(tempDir, "audio.mp3");
